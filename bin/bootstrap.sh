...
for f in $(grep -lr griftcard $PROJECT_PATH); do
    sed -e "s/griftcard/${PROJECT_NAME}/g" < $f > $f.tmp && mv $f.tmp $f
done

# replace all references to Griftcard in project files
for f in $(grep -lr Griftcard $PROJECT_PATH); do
    sed -e "s/Griftcard/${PROJECT_NAME_PROPER}/g" < $f > $f.tmp && mv $f.tmp $f
done

# rename project module directory
mv ${PROJECT_PATH}/griftcard ${PROJECT_PATH}/${PROJECT_NAME}

# echo blurb
echo "griftcarding of $PROJECT_NAME to $PROJECT_PATH was successful."
